...
| Validation posture | C+ | `scripts/validate.sh` covers formatting, `cargo check`, workspace tests, and docs hygiene; fixture tests cover the implemented CLI feedback loop; a two-instance rootless Podman managed-browser smoke has passed. | Real Chromium smoke coverage and broader host/container matrix evidence remain limited. |

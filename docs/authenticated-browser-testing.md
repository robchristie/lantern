...
- Interaction output: `click`, `type`, `key`, `hover`, `wheel`, and `drag` report metadata about dispatch and selected page state. They do not print DOM text, input values, headers, bodies, cookies, storage, screenshots, or a `--text-file` path/source. `type` still sends the supplied text to the page.

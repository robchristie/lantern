...
| I | Unique, scrolled, enabled, stable, hit-tested interactions; diagnostic failures; compatible opt-in strict exit semantics | `interaction.rs` and CLI contract: dispatch does not establish outcome; timeouts can succeed | 2: trustworthy interactions | Focus-delivery calibration on draft PR #10 |
...
| I: trustworthy interactions | Candidate identified by package PR / no downstream consumers yet | Unique, scrolled, stable hit-tested input; strict exits; 22 real-Chromium contracts with CDP input audit | `2026-09-07-trustworthy-interactions.md`; `../../testing/browser-contracts.md` | Focus-delivery calibration after macOS CI failure |
